...
def create_model(
        ...
        **kwargs):
    """Create a model
create_model('efficientnet_v2s', num_classes=101)
    Args:
    ...
    model_name（必填）：指定要实例化的模型的名称。
    pretrained：一个布尔值，如果为 True，则加载预训练的 ImageNet-1k 权重。
    checkpoint_path：用于加载模型初始化后的检查点路径。
    scriptable、exportable、no_jit：用于设置模型的层配置。
    drop_rate、global_pool：这些是关键字参数，可能是针对特定模型的参数设置。
    """
    source_name, model_name = split_model_name(model_name)

    # Only EfficientNet and MobileNetV3 models have support for batchnorm params or drop_connect_rate passed as args
    #只有 EfficientNet 和 MobileNetV3 模型支持 batchnorm 参数或作为参数传递drop_connect_rate
